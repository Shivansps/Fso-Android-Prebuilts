...
cmake .. -DFSO_BUILD_WITH_OPENGL_ES=ON -DFSO_BUILD_WITH_OPENXR=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=$ANDROID_NDK_HOME/build/cmake/android.toolchain.cmake -DANDROID_ABI=$TARGET_ABI -DANDROID_PLATFORM=$ANDROID_PLATFORM -DEMBEDFILE_PATH=../tool/bin/embedfile -DFSO_PREBUILT_OVERRIDE=$PREBUILT_FOLDER/$TARGET_ABI -G Ninja && sed -i 's/-lusb-1.0//' build.ninja
ninja
mkdir -p ../../jniLibs/"$TARGET_ABI"
cp -r bin/*.so  ../../jniLibs/"$TARGET_ABI"
cd ..
